...
function Auth() {
    ...
    return (
        <div className="auth-container"> {/* Tambahkan kelas auth-container */}
            <h2>{isLogin ? 'Login' : 'Register'}</h2>
            {user && <p>Welcome, {user.email}!</p>}
            <form onSubmit={handleSubmit}>
                <div className="input-container"> {/* Tambahkan div untuk input-container */}
                    <input
                        type="email"
                        placeholder="Email"
                        value={email}
            ...
            <button className="toggle-button" onClick={() => setIsLogin(!isLogin)}> {/* Tambahkan kelas toggle-button */}
                {isLogin ? 'Need an account? Register' : 'Have an account? Login'}
            </button>
            {error && <p style={{ color: 'red' }}>Error: {error}</p>} {/* Pesan kesalahan */}
        </div>
    );
}
